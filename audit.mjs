...
/* ---------------------------- 21. TOPIC LIST <-> SCHEMA PARITY
 * The chunk instruction says "INCLUDE ONLY the keys listed under GENERATE NOW".
 * A top-level schema key with no matching topic is therefore SUPPRESSED - the
 * model is told to omit a field the report then tries to render.
 *
 * That is what silently removed Survey's conclusion (3 topics for a 7-key
 * schema), Site Context's six sections, and Solar's PV section. Each list had
 * been generated by an automated pass that read the wrong schema block, and the
 * tools reported "all sections generated" because by their own short list they
 * were.
 */
{
  const bad = [];
  for (const t of TOOLS) {
    const m = A[t].match(/const INSIGHT_TOPICS = \[([\s\S]*?)\];/);
    if (!m) continue;
    const topics = [...m[1].matchAll(/key: "([a-z_]+)"/g)].map((x) => x[1]);
    // every topic must actually be requested somewhere in the prompt
    const body = A[t].slice(A[t].indexOf("const INSIGHT_TOPICS"));
    // Schemas sit inside JS strings with varying escape depth (\" vs \\").
    // Guessing the escaping produced phantom failures, so match the bare key
    // name adjacent to a quote or backslash instead.
    const unrequested = topics.filter((k) => !new RegExp('["\\\\]' + k + '["\\\\]').test(body));
    if (unrequested.length) bad.push(`${t}: ${unrequested.join(",")}`);
  }
  check("Topics", "every topic is a key the prompt actually asks for", bad.length === 0, bad.join(" | "));
}
check("Topics", "Survey requests all seven of its schema keys",
  (A.SurveyAnalyzer.match(/key: "[a-z_]+"/g) || []).length === 7);
check("Topics", "Solar requests its PV section",
  /key: "energy_potential"/.test(A.SolarAnalyzer));
check("Topics", "Site Context requests its six report sections",
  ["adjacencies","accessibility_standards","hazard_screening","quiet_and_active_zoning",
   "key_findings","constraints"].every((k) => new RegExp(`key: "${k}"`).test(A.SiteContextAnalyzer)));


/* ------------------------------ 21. GENERATED CONTENT REACHES THE REPORT
 * A section can be generated, merged, saved, listed as complete - and still not
 * appear, because the report reads a different key. Survey's [10] read
 * `analysis.conclusion`, a key its schema never produces; Wind, Vegetation and
 * Budget each generated a `conclusion` that [10] never included. The section
 * list and the report were both truthful about different things.
 */
{
  const withConclusion = ["WindAnalyzer", "VegetationAnalyzer", "BudgetTracker", "SolarAnalyzer"];
  const miss = withConclusion.filter((t) =>
    /key: "conclusion"/.test(A[t]) && !/insight\?\.conclusion|analysis\?\.conclusion|\.conclusion \? \[/.test(A[t]));
  check("Reports", "a generated conclusion reaches section 10", miss.length === 0, miss.join(", "));
  check("Reports", "Survey reads the key it actually produces",
    /analysis\?\.overall_summary/.test(A.SurveyAnalyzer));
}
check("Images", "photo notes are shown to the user, not just used silently",
  /What the AI saw in your photos/.test(A.VegetationAnalyzer));
check("Images", "photo notes are editable so a misread can be corrected",
  /onChange=\{\(e\) => setPhotoNotes\(e\.target\.value\)\}/.test(A.VegetationAnalyzer));
