        ...
        <div className="px-4 pb-3 space-y-2 text-xs text-brand-text">
          ...
          {spec.needsWebSearch && (
            <div className="border-t border-brand-border/60 pt-2 mt-1">
              ...
              <p className="mt-0.5">
                This tool researches your location from live sources rather than relying on the model's training
                knowledge. Both providers support this - Claude through its web search tool, Gemini through Google
                Search grounding - and the sources actually used are listed in the report's References section, so
                every researched claim can be traced.
              </p>
              <p className="mt-1 text-brand-warning">
                If a source list does not appear, the model answered from training knowledge instead. Treat those
                results as indicative and verify anything load-bearing.
              </p>
            </div>
          )}
