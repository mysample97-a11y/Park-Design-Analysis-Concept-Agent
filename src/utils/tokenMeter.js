 ...
 */
/*
 * WHY THE COUNTER JUMPS BY MORE THAN ONE PER CLICK
 *
 * Each RETRY is a real HTTP request and the provider counts it, so one click can
 * legitimately be two or three. The count was accurate but silent about the
 * reason, which made it look like a fault. Each request now records WHY, and the
 * rail shows the breakdown for the most recent run.
 */
const REASON_KEY = "as2p_reqlog_v1";

export function recordRequestReason(provider, reason) {
  try {
    const raw = localStorage.getItem(REASON_KEY);
    const log = raw ? JSON.parse(raw) : [];
    const list = Array.isArray(log) ? log : [];
    list.push({ at: Date.now(), provider, reason });
    // Only the last few matter; this is an explanation, not an audit trail.
    localStorage.setItem(REASON_KEY, JSON.stringify(list.slice(-12)));
  } catch { /* never fail a run over logging */ }
}

/** Requests in the last 2 minutes, grouped by reason. */
export function recentRequestReasons(now = Date.now()) {
  try {
    const raw = localStorage.getItem(REASON_KEY);
    const list = raw ? JSON.parse(raw) : [];
    if (!Array.isArray(list)) return [];
    const recent = list.filter((r) => r && now - r.at < 120000);
    const counts = {};
    recent.forEach((r) => { counts[r.reason] = (counts[r.reason] || 0) + 1; });
    return Object.keys(counts).map((k) => ({ reason: k, n: counts[k] }));
  } catch {
    return [];
  }
}
